...
logging.info(f"Connected to Master PC at {MASTER_PC_IP}:{MASTER_PC_PORT} with identity {identity.decode()}")
...
def apply_settings(settings):
    ...
    try:
        ...
        log_event("Camera settings applied.")
...
def send_status():
    while True:
        ...
        try:
            ...
            log_event("Sent STATUS message to Master.")
...
def handle_task(message):
    ...
    if not task:
        log_event(f"Ignored message without task: {message}")
        return
    if task == 'REC_START':
        ...
        try:
            ...
            log_event("Sent SYNC_RESPONSE to Master.")
        except zmq.ZMQError as e:
        ...
        try:
            ...
            log_event("Sent SETTINGS_UPDATED confirmation to Master.")
        except zmq.ZMQError as e:
            log_event(f"Failed to send SETTINGS_UPDATED confirmation: {e}")
    else:
        log_event(f"Received unknown task: {task}")
...
def run_tcp_server():
    ...
    if __name__ == '__main__':
        ...
        threading.Thread(target=run_zmq_listener, daemon=True).start()

        # Start sending status updates in a separate thread
        threading.Thread(target=send_status_periodically, daemon=True).start()
