...
# USER CONFIG
# -------------------
MAX_SIMULTANEOUS_DOWNLOADS = 10  # <--- Adjust to allow up to N cameras downloading in parallel
# -------------------
...
class SessionDownloaderApp:
    def __init__(self, master):
        ...
        # For parallel progress updating
        self.total_received_lock = threading.Lock()
    ...
    def create_widgets(self):
        ...
        # We remove the "current session progress" elements and only keep an Overall Progress
        self.overall_progress_label = ttk.Label(progress_frame, text="Overall Progress:")
        self.overall_progress_label.grid(row=0, column=0, padx=5, pady=5, sticky="w")

        self.overall_progress = ttk.Progressbar(progress_frame, length=400, mode='determinate')
        self.overall_progress.grid(row=1, column=0, padx=5, pady=5, sticky="ew")

        self.overall_eta_label = ttk.Label(progress_frame, text="ETA: N/A")
        self.overall_eta_label.grid(row=2, column=0, padx=5, pady=5, sticky="w")
    ...
    def get_sessions_thread(self):
        ...
        # Build up session_info data
        for session_name in self.sessions:
            self.session_info[session_name] = {"clip_sizes": {}}
            for cam in CAMERAS:
        ...
        self.schedule_refresh()  # re-schedule
    ...
    def update_session_tree(self):
        def update():
            ...
            for session_name in self.sessions:
                ...
                if found_sizes:
                    first_found_size = found_sizes[0]  # purely for display
    ...
    def determine_session_status(self, session_name, clip_sizes):
        ...
        # Check if each file is present with correct size
        available_count = 0
        ...
        for cam, sz in cameras_with_clips:
            ...
            if not ip:
                continue
            # We only check .h264 file size for status? 
            # But now we also have .jpg. 
            # The "clip_sizes" is the sum of all .h264 + .jpg. 
            # Minimal approach: if local folder has the same total size across matching files, we count it. 
            # Or we just see if the sum of .h264 + .jpg = clip_sizes[cam]? 
            # For simplicity, just do a naive check on .h264. 
            # A more robust approach is to sum local .h264 + .jpg. We'll keep it simple here:
            local_total = self.local_session_size_for_ip(session_name, ip)
            if abs(local_total - sz) < 2:  # allow for slight mismatch
                available_count += 1
    ...
    def local_session_size_for_ip(self, session_name, ip):
        """
        Sum size of all .h264/.jpg files in local Sessions folder that match session_name + lastOctet suffix
        """
        session_folder = os.path.join(SESSIONS_FOLDER, session_name)
        if not os.path.exists(session_folder):
            return 0
        ...
        for f in os.listdir(session_folder):
            if f.startswith(session_name + "_" + octet):
                fp = os.path.join(session_folder, f)
    ...
    def download_session_thread(self, session_name):
        ...
        # total_bytes is sum of all cameras' session_size
        total_bytes = sum(sz for _, _, sz in cameras_with_session)
        self.total_received = 0
        overall_start_time = time.time()

        # We'll parallelize downloads from each camera up to MAX_SIMULTANEOUS_DOWNLOADS
        def download_one_camera(cam_name, ip, file_size):
            ...
            try:
                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
                    ...
                    # We expect a list of "files": [{'filename':..., 'size':...}, ...]
                    ...
                    for fi in files_list:
                        ...
                        bytes_received_for_this_file = 0
                        with open(file_path, 'wb') as f:
                            while bytes_received_for_this_file < fsize:
                                data = sock.recv(min(4096, fsize - bytes_received_for_this_file))
                                if not data:
                                    break
                                f.write(data)
                                bytes_received_for_this_file += len(data)
                                with self.total_received_lock:
                                    self.total_received += len(data)
                                self.update_overall_progress(overall_start_time, self.total_received, total_bytes)

                        if bytes_received_for_this_file < fsize:
                            raise Exception(f"Connection lost during file transfer: {fname}")
                    logging.info(f"Downloaded session {session_name} from {ip} successfully.")

            except Exception as e:
                logging.error(f"Failed to download session '{session_name}' from {ip}: {e}", exc_info=True)
                # We'll not pop up a messagebox from each thread; you could do so if you want.
        ...
        with ThreadPoolExecutor(max_workers=MAX_SIMULTANEOUS_DOWNLOADS) as executor:
            ...
            for cam_name, ip, file_size in cameras_with_session:
                futures.append(executor.submit(download_one_camera, cam_name, ip, file_size))
            # Wait for them all to complete
            for f in futures:
                f.result()  # will raise any exception that occurred in the thread

        # Final update
        self.update_overall_progress(overall_start_time, total_bytes, total_bytes)
        messagebox.showinfo("Success", f"Session '{session_name}' downloaded successfully (where available).")
        self.update_session_tree()
    ...
    def delete_session_remote_thread(self, session_name):
        cameras_with_session = [
            cam for cam in CAMERAS
            if self.session_info.get(session_name, {}).get("clip_sizes", {}).get(cam['name'], None) is not None
        ]
        if not cameras_with_session:
        ...
        failed_cams = [f"{cam} ({res[1]})" for cam, res in delete_results.items() if not res[0]]

        if success_cams:
            msg = f"Successfully deleted session '{session_name}' on cameras:\n" + ", ".join(success_cams)
            messagebox.showinfo("Success", msg)
        if failed_cams:
            msg = f"Failed to delete session '{session_name}' on cameras:\n" + ", ".join(failed_cams)
    ...
    def update_overall_progress(self, start_time, total_received, total_bytes):
        """
        Update the overall progress bar and ETA as downloads proceed.
        Called by each camera download thread while reading chunks.
        """
        if total_bytes <= 0:
            return

        percentage = (total_received / total_bytes) * 100
