...
# Function to save settings to camera_settings.json
def save_settings():
    settings = {
        'width': int(width_entry.get()),
        'height': int(height_entry.get()),
        'frame_rate': int(fps_scale.get()),
        'shutter_angle': int(shutter_angle_scale.get()),
        'iso': int(iso_scale.get()),
        'brightness': int(brightness_scale.get()),
        'contrast': int(contrast_scale.get()),
        'saturation': int(saturation_scale.get()),
        'sharpness': int(sharpness_scale.get()),
        'auto_exposure': bool(ae_var.get()),
        'flicker_control': flicker_var.get(),
        'flicker_period': int(flicker_period_scale.get()),
        'white_balance': wb_var.get(),
        'red_gain': float(red_gain_scale.get()),
        'blue_gain': float(blue_gain_scale.get())
    }
    with open('camera_settings.json', 'w') as f:
        json.dump(settings, f)
    print("Settings saved to camera_settings.json")
...
# --- GUI Elements Creation (Unchanged) ---
# Resolution settings
...
fps_scale.set(default_settings['frame_rate'])
...
shutter_angle_scale.set(default_settings['shutter_angle'])
...
iso_scale.set(default_settings['iso'])
...
brightness_scale.set(default_settings['brightness'])
...
contrast_scale.set(default_settings['contrast'])
...
saturation_scale.set(default_settings['saturation'])
...
sharpness_scale.set(default_settings['sharpness'])
sharpness_scale.pack(fill="x", padx=5, pady=5)

# Auto Exposure
ae_var = tk.BooleanVar(value=default_settings['auto_exposure'])
...
flicker_var = tk.StringVar(value=default_settings['flicker_control'])
...
flicker_period_scale.set(default_settings['flicker_period'])

# Show or hide flicker period slider based on default setting
if default_settings['flicker_control'] == 'Manual':
    flicker_period_scale.pack(fill="x", padx=5, pady=5)
...
wb_var = tk.StringVar(value=default_settings['white_balance'])
...
red_gain_scale.set(default_settings['red_gain'])
...
blue_gain_scale.set(default_settings['blue_gain'])

# Show or hide red and blue gain sliders based on default setting
if default_settings['white_balance'] == 'Manual':
    red_gain_scale.pack(fill="x", padx=5, pady=5)
    blue_gain_scale.pack(fill="x", padx=5, pady=5)
...
# --- End of GUI Elements Creation ---

# Apply initial controls
update_controls()

# Configure and start the camera with initial settings
update_resolution()
