...
class SessionDownloaderApp:
    ...
    def create_widgets(self):
        ...
        # Store buttons as instance variables for later (disabling in offline mode)
        ...
        # OFFLINE MODE: Disable remote actions if running offline.
        if self.offline_mode:
    ...
    def get_sessions(self):
        """Spawn a background thread to query sessions so we don't block the GUI."""
        self.refresh_button.config(state="disabled")
        t = threading.Thread(target=self.get_sessions_thread, daemon=True)
        t.start()

    def get_sessions_thread(self):
        # OFFLINE MODE: Get sessions from the local sessions folder only.
        if self.offline_mode:
            local_sessions = []
            new_session_info = {}
    ...
    def determine_session_status(self, session_name, clip_sizes):
        # OFFLINE MODE: Always return "local only"
        if self.offline_mode:
            return "local only"
